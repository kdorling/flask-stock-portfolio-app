"""
This file (test_users.py) contains the functional tests for registering users.
"""
...
def test_valid_registration(test_client):
    ...
    THEN check the response is valid, the user is registered, and an email was
         queued up to send
    """
    with mail.record_messages() as outbox:
        ...
        assert response.status_code == 200
        assert (b'Thanks for registering, patrick@email.com! Please check '
                b'your email to confirm your email address.'
                in response.data)
        assert b'Flask Stock Portfolio App' in response.data
        assert len(outbox) == 1
        assert outbox[0].subject == ('Flask Stock Portfolio App - Confirm '
                                     'Your Email Address')
        assert outbox[0].sender == 'kdor.trades@gmail.com'
...
def test_invalid_registration(test_client):
    """
    GIVEN a Flask application configured for testing
    WHEN the '/users/register' page is posted to (POST) with invalid data
            (missing password)
    THEN check an error message is returned to the user
    """
    response = test_client.post('/users/register',
                                data={'email': 'patrick2@email.com',
                                      'password': ''},  # Empty pw not allowed!
                                follow_redirects=True)
    assert response.status_code == 200
    assert b'Thanks for registering, patrick2@email.com!' \
...
def test_duplicate_registration(test_client):
    """
    GIVEN a Flask application configured for testing
    WHEN the '/users/register' page is posted to (POST) with the email address
    ...
    test_client.post('/users/register',
                     ...
                     follow_redirects=True)
    response = test_client.post('/users/register',
                                data={'email': 'patrick@hotmail.com',  # Dupl.
                                      'password': 'FlaskIsStillGreat!'},
                                follow_redirects=True)
    assert response.status_code == 200
    assert b'Thanks for registering, patrick@hotmail.com!' \
        not in response.data
    assert b'Flask Stock Portfolio App' in response.data
    assert b'ERROR! Email (patrick@hotmail.com) already exists.' \
        in response.data
...
def test_get_login_page(test_client):
    ...
    assert b'Email' in response.data
    assert b'Password' in response.data
    assert b'Login' in response.data  # Button also called "Login"
...
def test_valid_login_when_logged_in_already(test_client, log_in_default_user):
    """
    GIVEN a Flask application configured for testing and the default user
          logged in
    WHEN the '/users/login' page is posted to (POST) with value credentials
...
def test_invalid_logout_not_logged_in(test_client):
    """
    GIVEN a Flask application configured for testing
    WHEN the '/users/logout' page is requested (GET) when the user is not
         logged in
...
def test_user_profile_logged_in_email_confirmed(test_client,
                                                confirm_email_default_user):
    """
    GIVEN a Flask application configured for testing and the default user
          logged in and their email address is confirmed
    WHEN the '/users/profile' page is requested (GET)
    ...
    assert b'Email address confirmed on Wednesday, July 08, 2020' \
        in response.data
    assert b'Account Actions' in response.data
    assert b'Change Password' in response.data
...
def test_user_profile_not_logged_in(test_client):
    """
    GIVEN a Flask application configured for testing
    WHEN the '/users/profile' page is requested (GET) when the user is NOT
         logged in
    THEN check that the user is redirected to the login page
    """
...
def test_navigation_bar_logged_in(test_client, log_in_default_user):
    ...
    THEN check that the 'List Stocks', 'Add Stock', 'Profile' and 'Logout'
         links are present
    """
...
def test_login_with_next_valid_path(test_client, register_default_user):
    """
    GIVEN a Flask application configured for testing
    WHEN the 'users/login?next=%2Fusers%2Fprofile' page is posted to (POST)
         with a valid user login
...
def test_login_with_next_invalid_path(test_client, register_default_user):
    """
    GIVEN a Flask application configured for testing
    WHEN the 'users/login?next=http://www.badsite.com' page is posted to (POST)
         with a valid user login
...
def test_confirm_email_valid(test_client):
    ...
    confirm_serializer = \
        URLSafeTimedSerializer(current_app.config['SECRET_KEY'])
    token = confirm_serializer.dumps('patrick@gmail.com',
...
def test_confirm_email_invalid(test_client):
    """
    GIVEN a Flask application configured for testing
    WHEN the '/users/confirm/<token>' page is is requested (GET) with invalid
         data
    THEN check that the link was not accepted
    """
    response = test_client.get('/users/confirm/bad_confirmation_link',
                               follow_redirects=True)
    assert response.status_code == 200
...
def test_get_password_reset_via_email_page(test_client):
    ...
    THEN check the response is valid
    """
    response = test_client.get('/users/password_reset_via_email',
                               follow_redirects=True)
    assert response.status_code == 200
...
def test_post_password_reset_via_email_page_valid(test_client,
                                                  confirm_email_default_user):
    """
    GIVEN a Flask application configured for testing
    WHEN the '/users/password_reset_via_email' page is posted to (POST) with a
         valid email address
    ...
    with mail.record_messages() as outbox:
        response = test_client.post('/users/password_reset_via_email',
                                    ...
                                    follow_redirects=True)
        assert response.status_code == 200
        assert b'Please check your email for a password reset link.' \
            in response.data
        assert len(outbox) == 1
        ...
        assert 'http://localhost/users/password_reset_via_token/' \
            in outbox[0].html
...
def test_post_password_reset_via_email_page_invalid(test_client):
    """
    GIVEN a Flask application configured for testing
    WHEN the '/users/password_reset_via_email' page is posted to (POST) with
         an invalid email address
    THEN check that an error message is flashed
    """
    with mail.record_messages() as outbox:
        response = test_client.post('/users/password_reset_via_email',
                                    data={'email': 'patrick@live.com'},
                                    follow_redirects=True)
        assert response.status_code == 200
        assert len(outbox) == 0
        print(response.data)
...
def test_post_password_reset_via_email_page_not_confirmed(test_client):
    """
    GIVEN a Flask application configured for testing
    WHEN the '/users/password_reset_via_email' page is posted to (POST) with a
         valid but unconfirmed email address
    THEN check that an error message is flashed
    """
    with mail.record_messages() as outbox:
        response = test_client.post('/users/password_reset_via_email',
        ...
        assert response.status_code == 200
        assert len(outbox) == 0
        assert b'Your email address must be confirmed before attempting a ' \
               b'password reset.' in response.data


def test_get_password_reset_valid_token(test_client):
    """
    GIVEN a Flask application configured for testing
    WHEN the '/users/password_reset_via_email/<token>' page is requested (GET)
         with a valid token
...
def test_post_password_reset_valid_token(
    ...
    """
    GIVEN a Flask application configured for testing
    WHEN the '/users/password_reset_via_email/<token>' page is posted to (POST)
         with a valid token
...
def test_post_password_reset_invalid_email(test_client):
    """
    GIVEN a Flask application configured for testing
    WHEN the '/users/password_reset_via_email/<token>' page is requested (GET)
         with a valid token but invalid email
    THEN check that an error message is displayed
    """
    password_reset_serializer = URLSafeTimedSerializer(
        current_app.config['SECRET_KEY']
...
def test_get_change_password_logged_in(test_client, log_in_default_user):
    ...
    """
    response = test_client.get('/users/change_password',
                               follow_redirects=True)
    assert response.status_code == 200
    assert b'Change Password' in response.data
...
def test_get_change_password_not_logged_in(test_client):
    """
    GIVEN a Flask application configured for testing with the user NOT logged
          in
    WHEN the '/users/change_password' page is retrieved (GET)
    THEN check an error message is returned to the user
    """
...
def test_post_change_password_logged_in_valid_current_password(
    ...
    """
    GIVEN a Flask application configured for testing with the user logged in
    WHEN the '/users/change_password' page is posted to (POST) with the
...
def test_post_change_password_logged_in_invalid_current_password(
    ...
    user = User.query.filter_by(email='patrick@gmail.com').first()
    assert not user.is_password_correct('FlaskIsStillAwesome456')
    assert user.is_password_correct('FlaskIsAwesome123')
...
def test_get_resend_email_confirmation_logged_in(
    ...
    with mail.record_messages() as outbox:
        response = test_client.get('/users/resend_email_confirmation',
                                   follow_redirects=True)
        assert response.status_code == 200
        assert b'Email sent to confirm your email address.  Please check ' \
               b'your email!' in response.data
        assert len(outbox) == 1
        assert outbox[0].subject == \
...
def test_get_resend_email_confirmation_not_logged_in(test_client):
    ...
    GIVEN a Flask application configured for testing with the user not logged
          in
    WHEN the '/users/resend_email_confirmation' page is requested (GET)
    THEN check that an email was not queued up to send and an error message
         is displayed
    """
    with mail.record_messages() as outbox:
        response = test_client.get('/users/resend_email_confirmation',
        ...
        assert response.status_code == 200
        assert b'Email sent to confirm your email address.  Please check ' \
               b'your email!' not in response.data
        assert len(outbox) == 0
        assert b'Please log in to access this page.' in response.data
