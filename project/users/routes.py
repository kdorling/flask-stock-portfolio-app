    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            try:
                ...
                flash(f'Thanks for registering, {new_user.email}! Please '
                      f'check your email to confirm your email address.',
                      'success')
                current_app.logger.info(f'Registered new user: '
                                        f'{form.email.data}!')
                ...
                flash(f'ERROR! Email ({form.email.data}) '
...
def login():
    ...
    if current_user.is_authenticated:
        ...
        current_app.logger.info(f'Duplicate login attempt by user: '
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            if user and user.is_password_correct(form.password.data):
                ...
                # If the next URL is not specified, redirect to the user
                # profile
                if not request.args.get('next'):
                    return redirect(url_for('users.user_profile'))

                # Process the query to determine if the user should be
                # redirected after logging in
                next_url = request.args.get('next')
                if urlparse(next_url).scheme != '' \
                   or urlparse(next_url).netloc != '':
                    current_app.logger.info(f'Invalid next path in login '
                                            f'request: {next_url}')
                    logout_user()
                    return abort(400)

                current_app.logger.info(f'Redirecting after valid login to: '
                                        f'{next_url}')
...
def confirm_email(token):
    try:
        ...
        current_app.logger.info(f'Invalid or expired confirmation link'
                                f'received from IP address: '
    ...
    if user.email_confirmed:
        ...
        current_app.logger.info(f'Confirmation link received for a confirmed '
                                f'user: {user.email}')
    else:
...
def change_password():
    ...
    if form.validate_on_submit():
        if current_user.is_password_correct(form.current_password.data):
            ...
            current_app.logger.info(f'Password updated for user: '
                                    f'{current_user.email}')
            return redirect(url_for('users.user_profile'))
        else:
            flash('ERROR! Incorrect user credentials!')
            current_app.logger.info(f'Incorrect password change for user: '
                                    f'{current_user.email}')
...
def resend_email_confirmation():
    ...
    flash('Email sent to confirm your email address.  '
          'Please check your email!', 'success')
    current_app.logger.info(f'Email re-sent to confirm email address for user:'
                            f'{current_user.email}')
    return redirect(url_for('users.user_profile'))
...
def generate_confirmation_email(user_email):
    ...
    confirm_url = url_for('users.confirm_email',
                          token=confirm_serializer
                          .dumps(user_email,
                                 salt='email-confirmation-salt'),
                          _external=True)

    return Message(subject='Flask Stock Portfolio App - Confirm Your Email '
                           'Address',
                   html=render_template('users/email_confirmation.html',
...
def generate_password_reset_email(user_email):
    ...
    password_reset_url = \
        url_for('users.process_password_reset_token',
                token=password_reset_serializer.dumps(
                    user_email,
                    salt='password-reset-salt'),
                _external=True)

    return Message(subject='Flask Stock Portfolio App - Password Reset '
                           'Requested',
...
@users_blueprint.route('/password_reset_via_token/<token>',
                       methods=['GET', 'POST'])
